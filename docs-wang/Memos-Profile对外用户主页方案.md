...
> 状态：方案设计中
> 决策日期：2026-09-13
> 核心定位：Profile 是每个用户的公开博客主页，不是 Home 的复制品。
...
因此 Profile 应该从“某个用户的 memo 列表页”升级为“对外用户主页 / 公开博客主页”。这个能力不只属于 admin，而应该属于项目里的每个用户：普通用户和管理员都可以拥有自己的公开主页，未登录访客和已登录用户都可以访问，并看到该用户愿意公开、且当前访问者有权限查看的内容。

Home 仍然负责完整个人数据、私有统计和个人回顾；Profile 负责公开表达和可选的匿名创作概况。

公开博客主页的基础边界：

- 每个用户都有自己的公开主页，路由形态沿用 `/u/:username`。
- 未登录访客可以访问公开实例上的用户主页。
- 已登录用户也可以访问其他用户的公开主页。
- 页面内容只展示当前访问者有权限看到的 memo。
- 默认核心内容是 `PUBLIC` memo；登录用户在现有权限模型下也可以看到允许其访问的 `PROTECTED` memo。
- `PRIVATE` memo 的内容、标签、位置、附件和链接永远不进入他人的公开主页。
- 用户本人访问自己的 Profile 时，也应该被视为“公开主页预览”，不要把 Home 的私有数据体验混入其中。
...
### 2.4 Memo Detail 已经是单篇 memo 详情页
...
这就是用户在列表里点击某条 memo 后进入的详细页。它适合作为公开博客里的“单篇文章详情页”：Profile 负责用户公开博客首页，Memo Detail 负责单篇 memo 的详情阅读、评论展示和分享链接访问。

因此后续博客化设计里，几个页面职责应该分开：

- `Home`：当前用户自己的工作台，包含私有 memo、完整统计、编辑和回顾。
- `Profile / Public Blog`：某个用户的公开博客主页，只展示当前访问者可见内容。
- `Memo Detail`：单篇 memo / 文章详情页。
- `Explore`：全站公开内容流。

## 三、产品定位

Profile 是每个用户的公开博客主页，应该分为两层。
